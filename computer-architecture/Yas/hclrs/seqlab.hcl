...
conditionsMet = [
	(icode == CMOVXX && ifun == 0) : 1;
	(icode == CMOVXX && ifun == 1) : C_SF || C_ZF;
	(icode == CMOVXX && ifun == 2) : C_SF;
	(icode == CMOVXX && ifun == 3) : C_ZF;
	(icode == CMOVXX && ifun == 4) : !C_ZF;
	(icode == CMOVXX && ifun == 5) : !C_SF;
	(icode == CMOVXX && ifun == 6) : !(C_SF || C_ZF);
	1: 0;
];
...
ALU_IN2 = [
	icode in {OPQ, RMMOVQ} : reg_outputB;
	1 : 0xbad;
];
